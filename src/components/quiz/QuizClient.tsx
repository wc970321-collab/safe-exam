...
import { supabase } from "@/lib/supabase"; // 1. 引入通讯员
...
export default function QuizClient({
  ...
  const [user, setUser] = useState<any>(null); // 2. 存用户信息
 ...
 const handleSubmit = async () => {
    ...
    return (
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8 text-center">
          {user ? (
            <div className="mb-4 text-xs text-green-600 bg-green-50 py-1 px-3 rounded-full inline-block">
              ✓ 已连接云同步 ({user.email})
            </div>
          ) : (
            <div className="mb-4 text-xs text-amber-600 bg-amber-50 py-1 px-3 rounded-full inline-block">
              未登录，进度仅保存在本地
            </div>
          )}
          <h2 className="text-xl font-bold text-gray-900 mb-4">章节练习</h2>
          <div className="grid grid-cols-3 gap-4 mb-6 max-w-sm mx-auto">
            ...
            <div className="bg-amber-50 rounded-lg p-3">
              <div className="text-2xl font-bold text-amber-600">{passingScore}%</div>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <QuizProgress current={currentIndex + 1} total={totalQuestions} answered={answeredCount} />
      <QuizQuestionCard
      ...
      <div className="flex items-center justify-between mt-6">
        <button
          ...
          className="px-4 py-2 text-sm font-medium text-gray-600 disabled:opacity-30 transition-colors"
        >
          ← 上一题
        </button>
        <span className="text-sm text-gray-400">{currentIndex + 1} / {totalQuestions}</span>
        {isLast ? (
          <button
            onClick={handleSubmit}
            disabled={!allAnswered}
            className={`px-5 py-2 text-sm font-medium rounded-lg transition-colors ${
              allAnswered ? "bg-green-600 text-white hover:bg-green-700" : "bg-gray-100 text-gray-400 cursor-not-allowed"
            }`}
          >
